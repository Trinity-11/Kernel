...
UART_ENIRQ  .proc
            ...
            LDA @lINT_MASK_REG1     ; Enable COM1 interrupts
            AND #~FNX1_INT04_COM1
            STA @lINT_MASK_REG1
...
WAITINT      
            ; LDA @lINT_PENDING_REG1  ; Check to see if IRQ4 is pending
            ; AND #FNX1_INT04_COM1
            ; CMP #FNX1_INT04_COM1
            ; BNE WAITINT
...
;
; Print all received characters from selected COM port to the screen
;
UART_ECRC   .proc
            PHP
...
GETLOOP     JSL UART_HASBYT         ; Check to see if there is data available
            BCC ENDLOOP             ; No... return to caller

            JSL UART_GETC
            JSL PUTC                ; And echo it to the screen
            
            BRA GETLOOP

ENDLOOP     LDA #']'                ; Close the packaet
            JSL PUTC
